local koma_rafte = " in the of name komarafte "
...
if not database:get("setnumbergp") then
  ...
  print("\027[" .. color.black[1] .. ";" .. color.green[2] .. "m\n➡➡ [•• ᴄᴏɴғɪʀᴍᴇᴅ ʟɪᴄᴇɴsᴇ ° ʙʏ koma °  ••]\n\027[00m")

print("\027[" .. color.black[1] .. ";" .. color.white[2] .. "m\n➡➡ [••ᴄʜᴀɴɴᴇʟ: @komarafte | ɪs sᴇɴᴅ ° ᴜᴘᴅᴇᴛᴇ ° ᴍᴏᴅʀɴ••] ▶\n\027[00m")

print("\027[" .. color.black[1] .. ";" .. color.white[2] .. "m\n➡➡ [••برای دریافت اپدیت ها  به کانال ما بزنید | ᴄʜᴀɴɴᴇʟ: @komarafte ••] ▶\n\027[00m")


local is_KOMA = function(msg)
  local var = false
  if msg.sender_user_id_ == tonumber(koma) then
    var = true
  end
  return var
end
...
The *Number* of *Warnings* user : `[]] .. warns .. "/" .. sencwarn .. "]`", 1, "md")
      ...
      if is_momod(msg.sender_user_id_, msg.chat_id_) and text:match("^[Ii]d (.*)$") and check_user_channel(msg) then
        local id_by_men = function(extra, result)
          if result.id_ then
            if database:get("lang:gp:" .. msg.chat_id_) then
              if tonumber(result.id_) == tonumber(koma) then
                t = "Developer"
...
If you *want* to get the bot phone number , send command *Botphone* .]], 1, "md")
        ...
        if is_KOMA(msg) and text:match("^[Ww]hat[Ss]ource$") then
          if database:get("lang:gp:" .. msg.chat_id_) then
            send(msg.chat_id_, msg.id_, 1, "》 *KOMA V.1*\n\n》 *Created on JUNE 2018*\n\n》 *By* : *KOMARAFTE* \n\n》 *Developer* :\n\n • KOMARAFTE\n\n  *AMIRREZA*", 1, "md")
          else
            send(msg.chat_id_, msg.id_, 1, "》 KOMA VERSION 1", 1, "md")
          end
        end
        if is_KOMA(msg) and text:match("^[Ii]nfo$") then
          send(msg.chat_id_, msg.id_, 1, c .. "\n\n> کد مجوز : " .. _config.License .. "\n> آی پی , 1, ", 1,"html")

          if is_leader(msg) and text:match("^[Ff]orceupdate$") then
            if not database:get("lang:gp:" .. msg.chat_id_) then
              send(msg.chat_id_, msg.id_, 1, "*•جهت اپدیت به کانال زیر مراجعه نمایید KOMARAFTE*: @KOMARAFTE", 1, "md")
            else
              send(msg.chat_id_, msg.id_, 1, "_• You are using the version provided by 'KOMARAFTE'_\n_Check out this Channel for updates :_ @KOMARAFTE", 1, "md")
            end
          end
